...
- 2026-08-27: Fixed homepage crash "TypeError: filter is not a function". Coerced fetched data to arrays (Array.isArray(r.data)?r.data:[]) at setState in Home.jsx & Shop.jsx; added guards on derived filters, offers product_ids.map, ProductDetail sizes.map + order() size lookup. Verified 100% (6/6) by testing_agent incl. simulated broken backend. Root architecture confirmed: CRA + FastAPI + MongoDB via REACT_APP_BACKEND_URL (backend healthy, returns 200 JSON arrays). No Vite/Render/Supabase in this codebase.
